...
v1.0.10
* fix: use `1 gwei` instead of `1 ether` as an 18 decimal threshold switch in the
...
v1.0.8:
* fix: don't allow `from > to` inputs into `random_bn(from, to)`
* fix: do not format simple number as strings in `print_amt()`

v1.0.7:
+ added: `gwei` constant equal to `10^9`
* fix: `print_amt()` to use 'b' suffix for a billion
...
+ added: `print_unix_ts()` function to print unit timestamp in a human-readable format
+ added: `print_duration()` function to print duration in seconds in a human-readable format

v1.0.1:
+ added: `ether` constant equal to `10^18`
